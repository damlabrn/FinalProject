...
def modelSolution():
    ...
    for (city_id1, city_id2), times in all_time:        
        time2.append([times]) 
    for (city_id1, city_id2), velocities in  all_velocity:            
        velocity2.append([velocities])
    for (city_id1), fuelPrices in  all_fuelPrice:            
        fuelPrice2.append([fuelPrices])
    for city_id1, city_id2, fuelConsumptions in  all_FuelConsumption:            
        fuelConsumption2.append([fuelConsumptions])
        
        
    total_cost = []
    for i in range(len(time2)):
        total_cost.append((time2[i][0]* alpha) + toll2[i][0])
   
    for i in range(len(total_cost)):
        all_cost_output.append([city_1[i], city_2[i], total_cost[i], pathDistance2[i][0], path2[i][0], toll2[i][0], time2[i][0]])
...
def costSolution():
    ...
    def get_min_cost(all_cost_output, a, b):
        sublists = [sublist for sublist in all_cost_output if sublist[0] == a and sublist[1] == b]
        if len(sublists) == 0:
            return None
        else:
            return min(sublists, key=lambda x: x[3])
    ...
    for i in IDs:
            for j in IDs:
                if i != j:
                    min_costOutput = get_min_cost(all_cost_output, i, j)
                    min_costOutput_list.append(min_costOutput)
        
    def saveCostSolutionOutput():
        df_SolutionMatrix = pd.DataFrame(min_costOutput_list)
        # define csv file name
        sol_file_name = 'solution_totalCost_' + now.strftime("%Y-%m-%d_%H-%M-%S") + '.csv'
            # set column names
        df_SolutionMatrix.columns = ['from', 'to', 'Total Cost', 'pathDistance', 'path', 'toll', 'time']
...
def Data_CSV():
    ...
    def saveDataOutput():
        df_dataMatrix = pd.DataFrame(all_cost_output)
        # define csv file name
        data_file_name = 'data_cost_' + now.strftime("%Y-%m-%d_%H-%M-%S") + '.csv'
            # set column names
        df_dataMatrix.columns = ['from', 'to', 'Total Cost', 'pathDistance', 'path', 'toll', 'time']
